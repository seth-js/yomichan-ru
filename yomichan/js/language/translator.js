    ...
    async _findTermsInternal(text, enabledDictionaryMap, options) {
        ...
        deinflections.forEach(flection => {
            ...
            databaseEntries.forEach(ent => {
                const {definitionTags, definitions}  = ent;

                if (definitionTags.includes('non-lemma')) {
                    ent.skip = true;
                    definitions.forEach(def => {
                        const reason = def.replace(/\sof\s.+/, '');
                        const lemma = def.replace(/.+?\sof\s/, '');

                        if (!lemmaMatchObj[lemma]) lemmaMatchObj[lemma] = [reason];
                        else if (!lemmaMatchObj[lemma].includes(reason)) lemmaMatchObj[lemma].push(reason);
                    });
                }
            });
        });
        ...
        Object.entries(lemmaMatchObj).forEach(ent => {
            const [lemma, reasons] = ent;
            lemmaMatches.push([lemma, reasons]);
        })
        
        const extraDeinflections = [];
        
        if (lemmaMatches.length > 0) {
            for (const [lemma, reasons] of lemmaMatches) {
