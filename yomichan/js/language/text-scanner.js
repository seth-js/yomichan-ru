...
class TextScanner extends EventDispatcher {
    ...
    async _findTermDictionaryEntries(textSource, optionsContext) {
        ...
        // Custom edits =================
        // This searches words, and their lowercase form so a search like Он and он will both be correctly searched.
        ...
        const normalSearch = await yomichan.api.termsFind(searchText, details, optionsContext);
        if (normalSearch.dictionaryEntries.length > 0) {
            if (!JSON.stringify(dictionaryEntries).includes(JSON.stringify(normalSearch.dictionaryEntries))) {
                dictionaryEntries.push(...normalSearch.dictionaryEntries);
                originalTextLength = normalSearch.originalTextLength;
            }
        }

        const lowerSearch = await yomichan.api.termsFind(searchText.toLowerCase(), details, optionsContext);
        if (lowerSearch.dictionaryEntries.length > 0) {
            if (!JSON.stringify(dictionaryEntries).includes(JSON.stringify(lowerSearch.dictionaryEntries))) {
                dictionaryEntries.push(...lowerSearch.dictionaryEntries);
                originalTextLength = lowerSearch.originalTextLength;
            }
        }
